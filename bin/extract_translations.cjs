...
const APP_ID = "frontend_insight";

async function run() {

  const files = [
      ...await glob("src/**/*.svelte"),
      ...await glob("src/**/*.ts")
      ]

  // Match translate("frontend_insight", "text") or translate('frontend_insight', 'text')
  // Using character class ["\'] to match either single or double quotes
  const translateRegexp = new RegExp(`translate\\(APP_ID,\\s*["\']([^"\']+)["\']`, "mg");

  // Match translatePlural("frontend_insight", "singular", "plural", ...) with either quote style
  const translatePluralRegexp = new RegExp(`translatePlural\\(APP_ID,\\s*["\']([^"\']+)["\'],\\s*["\']([^"\']+)["\']`, "mg");
  ...
  englishLabels.forEach(translation => {
    result.push(`const label${counter} = t('${APP_ID}', '${translation.replace(/'/g, "\\'")}');`);
    counter++;
  });
  ...
  pluralLabels.forEach(({ singular, plural }) => {
    result.push(`const label${counter} = n('${APP_ID}', '${singular.replace(/'/g, "\\'")}', '${plural.replace(/'/g, "\\'")}', 1);`);
    counter++;
  });
