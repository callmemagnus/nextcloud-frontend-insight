...
const sendUnhandledRejection = loadState(
  APP_ID,
  "collect_unhandled_rejections",
  true,
);

console.log(
  `${APP_ID} is enabled, sending error: ${sendError}, unhandledrejection: ${sendUnhandledRejection})`,
);
...
async function plan(e: ErrorEvent | PromiseRejectionEvent) {
  ...
  if (e.type === "unhandledrejection") {
    ...
    if (typeof ev.reason === "object") {
      ...
      payload.message = ev.reason;
    } else {
      payload.message = "No mesage in reason";
    }
